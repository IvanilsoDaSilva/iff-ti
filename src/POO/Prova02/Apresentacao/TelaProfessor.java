...
public class TelaProfessor extends JFrame{
	//atributos
	private JLabel lNome,lMatricula,ltitulacao,lEmail,ldtadmissao,lrua,lnumeroendereco,lcomplemento,lbairro,lcidade,lcep,lestado,lddi,lddd,lnumerotelefone,ltipo,loperadora,ldtemprestimo,ldtdevolucao,lcodigo,ltitulo,lautor,leditora,lano,lprecounitario,lprazo;
	private JButton BtnLimpar,BtnSalvar,BtnListar,BtnBuscar,BtnExcluir,BtnEditar,BtnFechar;
	private JTextField tNome,tMatricula,ttitulacao,tEmail,tdtadmissao,trua,tnumeroendereco,tcomplemento,tbairro,tcidade,tcep,testado,tddi,tddd,tnumerotelefone,ttipo,toperadora,tdtemprestimo,tdtdevolucao,tcodigo,ttitulo,tautor,teditora,tano,tprecounitario,tprazo;
	private JPanel pProfessor,pTelefone,pEndereco,pEmprestimo,pLivro;
	
	//vari�veis locais para armazenar o que for digitado
	String nome,matricula,titulacao,email,dtadmissao,rua,numeroendereco,complemento,bairro,cidade,cep,estado,ddi,ddd,numerotelefone,tipo,operadora,dtemprestimo,prazo,dtdevolucao,codigo,titulo,autor,editora,ano,precounitario;
	...
	public TelaProfessor() {
		...
		this.setTitle("Biblioteca - Professor");
		...
		pProfessor = new JPanel();
		pProfessor.setSize(860,90);
		pProfessor.setLocation(10,10);
		pProfessor.setBorder(BorderFactory.createTitledBorder("DADOS DO PROFESSOR:"));
		...
		ltitulacao = new JLabel("Titula��o: ");
		ltitulacao.setSize(260,30);
		ltitulacao.setLocation(5,50);
		
		ttitulacao = new JTextField();
		ttitulacao.setSize(277,20);
		ttitulacao.setLocation(65,55);
		...
		ldtadmissao = new JLabel("Data de Admiss�o: ");
		ldtadmissao.setSize(260,30);
		ldtadmissao.setLocation(600,20);
		
		tdtadmissao = new JTextField();
		tdtadmissao.setSize(90,20);
		tdtadmissao.setText("dd/mm/yyyy");
		tdtadmissao.setLocation(724,25);
		...
		class BatSinal extends MouseAdapter {
			public void mouseClicked(MouseEvent e) {
				if (e.getSource() == BtnSalvar) {	
					...
					titulacao = ttitulacao.getText();
					dtadmissao = tdtadmissao.getText();
					Pr = new Professor(nome,titulacao,matricula,email,dtadmissao,end,fone,Em,Lv);			
					}	
				...
				if (e.getSource() == BtnBuscar){	
					...
					if (matricula.equalsIgnoreCase("")){	
						...
						Professor objPr = new Professor();
						//objPr = (Professor) Professor.buscar(matricula); esse metodo requer conex�o com o BD
						
						JOptionPane.showMessageDialog(null,"ENDERE�O COMPLETO:\n\n"+objPr.getAtRefEndereco().getRua()+", "+objPr.getAtRefEndereco().getNumero()+" - "+objPr.getAtRefEndereco().getComplemento()+" - "+objPr.getAtRefEndereco().getBairro()+" - "+objPr.getAtRefEndereco().getCidade()+"/"+objPr.getAtRefEndereco().getEstado()+" - CEP: "+objPr.getAtRefEndereco().getCep());
						JOptionPane.showMessageDialog(null,"TELEFONE COMPLETO:\n\n"+objPr.getAtRefTelefone().getDdi()+" ("+objPr.getAtRefTelefone().getDdd()+") "+objPr.getAtRefTelefone().getNumero()+" - "+objPr.getAtRefTelefone().getTipo()+" - "+objPr.getAtRefTelefone().getOperadora());		
							}		
						}
		...
		//panel Professor
		this.add(pProfessor);
		pProfessor.add(lNome);
		pProfessor.add(tNome);
		pProfessor.add(lMatricula);
		pProfessor.add(tMatricula);
		pProfessor.add(lEmail);
		pProfessor.add(tEmail);
		pProfessor.add(ltitulacao);
		pProfessor.add(ttitulacao);
		pProfessor.add(ldtadmissao);
		pProfessor.add(tdtadmissao);
